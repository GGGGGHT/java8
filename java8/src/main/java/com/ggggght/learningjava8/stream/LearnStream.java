...
public class LearnStream {
  ...
  public void filterTest() {
    userList.stream()
        .filter(user -> Objects.equals("男", user.getSex()))
  ...
  public void countTest() {
    long count =
        userList.stream().filter(u -> u.getAge() > 20).filter(u -> Objects.equals(u.getSex(), "男"))
            .count();
  ...
  public void test() {
    ...
    Optional<User> max =
        userList.stream().max((e1, e2) -> Double.compare(e1.getAge(), e2.getAge()));
  ...
  public void peekTest() {
    ...
    List<Integer> res =
        nums.stream().peek(i -> System.out.println("from stream: " + i)).map(i -> i + 17)
  ...
  public void flatMapTest() {
    ...
    List<int[]> res =
        l1.stream().flatMap(i -> l2.stream().map(j -> new int[] {i, j})).collect(toList());
  ...
  public void getMaxByGrouping() {
    ...
    Map<String, Double> users =
        userList.stream().collect(groupingBy(User::getSex, averagingInt(User::getAge)));
  ...
  public void useParallel() {
    ...
    // 共享可变状态会影响并行流以及并行计算.
    LongStream.rangeClosed(1,100_000_000).parallel().forEach(accumulator::add);
