...
public class InsertSort {
	...
	public static void main(String[] args) {
		...
		/*
		 * int[] arr = new int[80000]; for (int i = 0; i < 80000; i++) { arr[i] = (int)
		 * (Math.random() * 8000000); } Instant start = Instant.now(); myInsert(arr);
		 * Instant end = Instant.now(); System.out.println(Duration.between(start,
		 * end).toMillis());
		 */
	}
