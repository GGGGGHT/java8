 ...
 * 问题:
 * - CAS 是什么？为什么要用到 CAS？
 * 答: CAS,Compare And Swap,即比较并交换.Java中的同步组件中大量使用了CAS实现了Java多线程的并发操作.整个AQS同步组件等都是以CAS来实现的.
 *
 * - CAS 和 synchronized的区别
 * 答: sync是线程获取锁是一种悲观锁策略,即假设每一次执行临界区代码会产生冲突,所以当前线程获取到锁之后会阻塞其他线程获取该锁.
 * CAS(无锁操作)是一种乐观锁策略,它假设所有线程访问共享资源的时候不会出现冲突,所以出现冲突时就不会阻塞其他的操作,而是重试当前操作直到没有冲突为止
 *
 * - CAS 原子操作的原理
 * 答: 使用一个期望值和一个变量的当前值进行比较，如果当前变量的值与我们期望的值相等，就使用一个新值替换当前变量的值。
 *
 * - CAS 有哪些问题？
 * 答: CAS可能会造成ABA问题 CAS 需要检查操作值有没有发生改变，如果没有发生改变则更新。但是存在这样一种情况：如果一个值原来是 A，变成了 B，然后又变成了 A，那么在 CAS 检查的时候会认为没有改变，但是实质上它已经发生了改变，这就是 ABA 问题。
 ...
 *
 * - Java 中利用 CAS 的原子操作有哪些，如何使用？
 * 答: 在J.U.C.atomic包下有许多原子类.有对包装类型操作的如:AtomicInteger,AtomicLong等.也有对引用类型操作的如AtomicReference
 *
 * 总结:
