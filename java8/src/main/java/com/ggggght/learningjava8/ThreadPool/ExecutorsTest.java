...
class MyTreadPool extends ThreadPoolExecutor {
	 ...
	 * @param workQueue the queue to use for holding tasks before they are executed. This
	 * queue will hold only the {@code Runnable} tasks submitted by the {@code execute}
	 * method.
	 ...
	 */
	public MyTreadPool(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
			BlockingQueue<Runnable> workQueue) {
		super(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue, new MyThreadFactory(),
				new ThreadPoolExecutor.CallerRunsPolicy());
	}

	/**
	 * Method invoked prior to executing the given Runnable in the given thread. This
	 * method is invoked by thread {@code t} that will execute task {@code r}, and may be
	 * used to re-initialize ThreadLocals, or to perform logging.
	 *
	 * <p>
	 * This implementation does nothing, but may be customized in subclasses. Note: To
	...
	private static class MyThreadFactory implements ThreadFactory {
		...
		/**
		 * Constructs a new {@code Thread}. Implementations may also initialize priority,
		 * name, daemon status, {@code ThreadGroup}, etc.
		 * @param r a runnable to be executed by new thread instance
		 * @return constructed thread, or {@code null} if the request to create a thread
		 * is rejected
		 */
		@Override
