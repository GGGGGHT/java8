...
/**
 * @desc: guava concurrency
 * 原始的{@link Future} 代表了一个异步计算的结果 可能已经或可能尚未完成生成结果的计算。 未来可以作为进行中计算的句柄，是服务向我们提供结果的承诺。
 * {@link ListenableFuture} 可以注册一个回调函数，会在计算执行结束后被立即执行
 * 可以通过{@method addListener}
 * 与Future相同 只有在get时 才会将异常输出
 * {@link Service} 代表了操作的状态
 */
public class GuavaConcurrency {
    ...
    /**
     * 使用{@link ListenableFuture} 的最重要原因是可以拥有复杂的异步操作链。
     * 避免嵌套使用Future
