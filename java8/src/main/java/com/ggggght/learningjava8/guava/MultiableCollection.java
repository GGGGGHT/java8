...
public class MultiableCollection {
    ...
    public void sortedMultiSetTest() {
        ...
        System.out.println("sortedMultiset.subMultiset(\"A\", BoundType.CLOSED, \"C\", BoundType.CLOSED).size() = " + sortedMultiset.subMultiset("A", BoundType.CLOSED, "C", BoundType.CLOSED).size());
    }
     ...
     * @method: clear()
     * @method: containsKey(key) Multimap.containsKey(key) is true if and only if there are any elements associated with the specified key.
     *          In particular, if a key k was previously associated with one or more values which have since been removed from the multimap,
     *          Multimap.containsKey(k) will return false. 当key有元素时才返回true 调用完clear()后就返回false
     */
    @Test
