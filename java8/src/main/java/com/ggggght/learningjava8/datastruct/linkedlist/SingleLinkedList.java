...
public class SingleLinkedList {
	...
	/**
	 * 查找单链表中倒数第K个节点 k是指链表的索引(即从0开始)不包括头节点 比如说有5个节点 需要倒数第5个 即是第1个节点
	 * @param args
	 */
	...
	/**
	 * 腾讯笔试题:反转单链表 反转一个单链表 0 -> 1 -> 2 -> 3 0 -> 3 -> 2 -> 1
	 * @param node
	 * @return
	 */
	...
	/**
	 * 给定一个链表的头结点 head,以及两个整数 from 和 to ,在链表上把第 from 个节点和第 to 个节点这一部分进行翻转。 例如： 给定如下链表，from
	 * = 2, to = 4 head-->5-->4-->3-->2-->1 将其翻转后，链表变成 head-->5--->2-->3-->4-->1
	 */
	...
	/**
	 * 百度笔试题: 逆序打印单链表 要求不破坏原来的数据结构 思路:借助于栈来实现操作
	 * @param args
	 */
	...
	/**
	 * 合并两个有序的单链表,要求合并后的结果依然有序 例如: n1: 1 -> 2 -> 4 n2: 2 -> 3 -> 5 res: 1 -> 2 -> 2 -> 3
	 * -> 4 -> 5
	 * @param args
	 */
	 ...
	 * 的整数倍，那么将最后剩余节点保持原有顺序。 示例 : 给定这个链表：head-->1->2->3->4->5 当 k = 2 时，应当返回:
	 * head-->2->1->4->3->5 当 k = 3 时，应当返回: head-->3->2->1->4->5
	 * @param args
	 */
	...
	/**
	 * 给定一个单链表，设计一个算法实现链表向右旋转 K 个位置。 举例：给定 head->1->2->3->4->5->NULL, K=3, 右旋后即为
	 * head->3->4->5-->1->2->NULL
	 */
	...
	public static void main(String[] args) {
		...
		/*
		 * list.addTail(heroNode3); list.addTail(heroNode1); list.addTail(heroNode4);
		 * list.addTail(heroNode5); list.addTail(heroNode2);
		 */
		addHead(heroNode1);
		addHead(heroNode2);
		addHead(heroNode3);
		...
		System.out.println();
		/*
		 * groupReverse(head, 2);// 0 --> 5 --> 4 --> 3 --> 2 --> 1 期望 0 --> 4 --> 5 --> 2
		 * --> 3 --> 1 System.out.println("----K反转后-------"); show();
		 */
		/*
		 * reverseRegion(head, 2, 4); // 0 --> 5 --> 4 --> 3 --> 2 --> 1 期望 0 --> 5 --> 2
		 * --> 3 --> 4 --> 1 System.out.println("----逆向一段后-------"); show();
		 */
		/*
		 * HeroNode heroNode = reverseNodeRecurive(head.next); head.next = heroNode;
		 * System.out.println("----递归逆序后-------"); show();
		 */

		/*
		 * heroNode5 = new HeroNode(2, "2", "222"); list.update(heroNode5);
		 * System.out.println("----更新完成后------"); show(); list.remove(1);
		 * System.out.println("----删除1完成后------"); show(); list.remove(5);
		 * System.out.println("----删除5完成后------"); show();
		 */
		/*
		 * System.out.println("有效节点的个数为:"+ list.size()); System.out.println("倒数第1个节点为" +
		 * getKNode(1));// HeroNode{no=5, name='tq', nickName=''}
		 * System.out.println("倒数第3个节点为" + getKNode(3));// HeroNode{no=3, name='ww',
		 * nickName=''}--->
		 */
		/*
		 * reverseNode(head); System.out.println("----反转完成后-------"); show();
		 */
		/*
		 * System.out.println("逆序打印单链表"); reversePrint(head);
		 */
	}
