 ...
 * </ul>
 * <div><div>Related Topics</div><div><li>贪心</li><li>数组</li><li>矩阵</li></div></div><br><div><li>👍 171</li><li>👎 0</li></div>
 */
...
public class Leetcode807 {
  public int maxIncreaseKeepingSkyline(int[][] grid) {
    ...
    int row = grid.length;
    int[] west = new int[row];
    int[] south = new int[column];
    ...
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < column; j++) {
        south[j] = Math.max(south[j], grid[i][j]);
    ...
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < column; j++) {
        int min = Math.min(west[i], south[j]);
        if (grid[i][j] < min) {
          res += min - grid[i][j];
        }
      }
