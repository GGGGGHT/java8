...
/**
 ...
 <div><div>Related Topics</div><div><li>字符串</li><li>模拟</li></div></div><br><div><li>👍 7</li><li>👎 0</li></div>
 */

public class Leetcodee2075 {
  public static String decodeCiphertext(String encodedText, int rows) {
    if (rows == 1) return encodedText;

    StringBuilder builder = new StringBuilder();
    char[] chars = encodedText.toCharArray();
    char[][] c = new char[rows][chars.length / rows];
    int idx = 0;
    int characters = 0;
    for (int i = 0; i < c.length; i++) {
      for (int j = 0; j < chars.length / rows; j++) {
    ...
    while (characters > 0) {
      ...
      for (int j = 0; j < rows; ) {
        ...
        if (Character.isLetter(c[j][j + t])) {
          characters--;
        }
        builder.append(c[j][j + t]);
        j++;
      }
      startC += 1;
    }
