...
public class LearnLambda {
  ...
  public void filterApples() {
    ...
    // Collection<Apple> greenApple = doFilterApples(apples, LearnLambda::isGreenApple);
    Collection<Apple> greenApple =
        doFilterApples(apples, a -> Objects.equals(a.getColor(), "green"));
    Preconditions.checkState(greenApple.size() == 2, "过滤绿色苹果出错");
    // Collection<Apple> heavyApple = doFilterApples(apples, LearnLambda::isHeavyApple);
    Collection<Apple> heavyApple = doFilterApples(apples, a -> 150 < a.getWeight());
    Preconditions.checkState(heavyApple.size() == 1, "过滤重苹果出错");
    Collection<Apple> greenAndHeavyApple =
        doFilterApples(apples, a -> 150 < a.getWeight() && Objects.equals(a.getColor(), "green"));
  ...
  public Collection<Apple> doFilterApples(Collection<Apple> inventory, Predicate<Apple> p) {
    if (CollectionUtils.isEmpty(inventory)) return inventory;
