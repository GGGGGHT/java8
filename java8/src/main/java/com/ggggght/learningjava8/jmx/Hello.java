...
public class Hello extends NotificationBroadcasterSupport implements HelloMBean {
	 ...
	 * attribute value never changes, but for other attributes it might change as the
	 * application runs. Consider an attribute representing statistics such as uptime or
	 * memory usage, for example. Being read-only just means that it can't be changed
	...
	/*
	 * Getter for the CacheSize attribute. The pattern shown here is frequent: the getter
	 * returns a private field representing the attribute value, and the setter changes
	 * that field.
	 */
	@Override
	...
	public synchronized void setCacheSize(int size) {
		 ...
		 * notification is the ObjectName of the MBean that emitted it. But an MBean can
		 * put a reference to itself ("this") in the source, and the MBean server will
		 * replace this with the ObjectName before sending the notification on to its
		 * clients.
		 *
		 * For good measure, we maintain a sequence number for each notification emitted
		 * by this MBean.
		 *
		 * The oldValue and newValue parameters to the constructor are of type Object, so
		 * we are relying on Tiger's autoboxing here.
		 */
		...
		/*
		 * Now send the notification using the sendNotification method inherited from the
		 * parent class NotificationBroadcasterSupport.
		 */
		sendNotification(n);
	}
